   ...
   if (!empty($item_name)){
    $query .= "AND i.item_id = '$item_name'";
   }
   $query .= "ORDER BY i.logs_timestamp DESC ";
...
<form method="POST" action="index_admin.php?page=items_reports">
                ...
                <select style="width: 250px;height:30px;margin-right:10px;"  name="item_name_dropdown" id="item_name_dropdown">
                    <option value="">ALL</option>
                      <?php
                        while ($row_itemName = $item_name_dropdown_value->fetch_assoc()):
                            ?>
                          <option value="<?php echo $row_itemName['item_id']; ?>"
                            <?php echo ($row_itemName['item_name'] == $item_name) ? 'selected' : ''; ?>>
